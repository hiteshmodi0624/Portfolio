...
const Work:FC<{work:work}>=({work})=>{
    ...
    return (
        <Link href={work.link} className={`relative rounded-full overflow-hidden h-48 w-48 flex 
                    items-center text-center justify-center border-2 hover:border-tertiary1 p-6 ${bg}`}>
            <Image
                src={work.image}
                height={200}
                width={200}
            ...
            hover:bg-opacity-95 text-2xl justify-center flex items-center text-tertiary1 opacity-0 hover:opacity-100 duration-300">{work.name}</span>
        </Link>
    );
}
export default Work
